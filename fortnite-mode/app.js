...
function playerOneController() {
    ...
    function gameLoop() {
        ...
        if(timePlaying%300 === 0){
            let height = $(".storm").css("height");
            height = parseInt(height);
            height+=20;

            $(".storm").animate({height: height+"px"});
        }  
        const height = parseInt($(".storm").css("height"))
        if (Top < height){
            if(timePlaying%100 === 0){
                health --;
        ...
        document.addEventListener('keypress', function (event) {
            if (event.key === "d" && reloading === false) {
                reloading = true
